  ...
  const ScrollHorizontal = (scrollOffset) => {
    ...
    return(
        <div className='w-full flex flex-col items-center'>
            ...
            {data &&(
                <div ref={menuRef} className='snap-x scroll-smooth w-[90%] grid max-sm:w-[350px] max-sm:h-[470px] z-10 max-sm:items-center max-sm:shadow-[inset_0px_2px_10px_0px_black] max-sm:rounded-xl grid-cols-4 max-sm:flex max-sm:flex-row md:gap-5 w-[90%] h-96 mt-6 max-sm:overflow-x-auto max-sm:overscroll-x-contain'>
                         ...
                         <div  className='inline-block snap-center bg-[#FFF8EA] max-sm:w-[300px] max-sm:mx-6 max-sm:px-4 shadow-[3px_2px_6px_2px_#594545] rounded-2xl h-[670px] max-sm:h-[370px] transform hover:scale-110 hover:cursor-pointer transition duration-500'>
                          ...
                          <h2 className='text-lg max-sm:text-sm z-10 font-bold ml-2 mt-6 text-[#815B5B] max-sm:hidden'>Contain:</h2>
                              <div>
                                ...
                                <ul className={`max-sm:grid max-sm:grid-cols-2 ${view === indexUtama ? 'bg-[#594545]' : 'bg-none'} max-sm:rounded-xl max-sm:w-[300px] max-sm:bottom-10 max-sm:right-4  max-sm:p-2 max-sm:relative max-sm:z-10`}>                    
